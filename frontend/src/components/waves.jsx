...
const Waves = ({
  ...
  useLayoutEffect(() => {
    ...
    function setSize() {
      ...
      // guard zero on first paint
      if (b.width === 0 || b.height === 0) return;

      boundingRef.current = { width: b.width, height: b.height, left: b.left, top: b.top };

      // DPR aware for crisp lines
      const dpr = Math.max(1, Math.min(window.devicePixelRatio || 1, 2));
      const w = Math.max(1, Math.floor(b.width * dpr));
      const h = Math.max(1, Math.floor(b.height * dpr));
    ...
    reflow();
    // one more pass next frame (covers font/layout late changes)
    rafSizeRef.current = requestAnimationFrame(reflow);

    // ---- start RAF loop (avoid double loops in StrictMode) ----
    ...
    return () => {
      ...
      if (ro) ro.disconnect();
      else window.removeEventListener("resize", reflow);
