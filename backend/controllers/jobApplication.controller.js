...
export async function submitJobApplication(req, res, next) {
  try {
    ...
    if ((parsed.coverNote || "").toLowerCase().includes("http://") || (parsed.coverNote || "").toLowerCase().includes("https://")) {
    ...
    if (recipients.length) {
      ...
      const html = `
        <div style="font-family:system-ui,Segoe UI,Roboto,Arial,sans-serif;font-size:14px;color:#222">
          ...
          ${doc.coverNote ? `<p style="margin-top:10px"><strong>Cover Note:</strong><br/>${escapeHtml(doc.coverNote).replace(/\n/g, "<br/>")}</p>` : ""}
      ...
      await sendMail({ to: recipients.join(","), subject, html, attachments });
    }

    // Optional applicant acknowledgement (non-fatal)
    if (process.env.SEND_APPLICANT_ACK === "true") {
      try {
        ...
        console.warn("Applicant ACK failed:", e?.message);
      }
    }

    return res.status(201).json({ message: "Application submitted", id: doc._id });
  } catch (err) {
    if (err?.code === "LIMIT_FILE_SIZE") return res.status(413).json({ error: "File too large (max 10MB)" });
    if (err?.issues) return res.status(400).json({ error: err.issues[0]?.message || "Invalid data" });
    return next(err);
  }
}
...
export async function adminListApplications(req, res) {
  ...
  const [items, total] = await Promise.all([
    JobApplication.find(filter).sort({ createdAt: -1 }).skip((pageNum - 1) * limitNum).limit(limitNum).lean(),
...
// ADMIN: download resume securely
